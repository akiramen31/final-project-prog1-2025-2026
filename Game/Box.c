...
void LoadBox(void)
{
	...
	{
		box[i] = (Box){ 0 };
		box[i].down.sprite = CreateSprite(texture, (sfVector2f) { SCREEN_WIDTH, SCREEN_HEIGHT }, scale, 41.f);
		sfSprite_setTextureRect(box[i].down.sprite, (sfIntRect) { 0, 4, 16, 16 });
		box[i].down.animation = (Animation){ (sfIntRect) { 0, 4, 16, 16 }, sfFalse, 4, 1.f, 0.f };
		...
		box[i].up.animation = (Animation){ (sfIntRect) { 0, 0, 16, 6 }, sfFalse, 4, 1.f, 0.f };
	}
}
