/*
	cute_tiled.c - Implementation of cute_tiled
	See cute_tiled.h for documentation and license.
*/
...
typedef struct strpool_embedded_t
{
	void* memctx;
	int ignore_case;
	int counter_shift;
	...
	strpool_embedded_internal_hash_slot_t* hash_table;
	int hash_capacity;

	strpool_embedded_internal_entry_t* entries;
	int entry_capacity;
	int entry_count;

	strpool_embedded_internal_handle_t* handles;
	int handle_capacity;
	int handle_count;
	int handle_freelist_head;
	int handle_freelist_tail;

	strpool_embedded_internal_block_t* blocks;
	int block_capacity;
	int block_count;
	int current_block;
} strpool_embedded_t;

static strpool_embedded_config_t const strpool_embedded_default_config =
{
	/* memctx         = */ 0,
	/* ignore_case    = */ 0,
	/* counter_bits   = */ 32,
...
static void strpool_embedded_init(strpool_embedded_t* pool, strpool_embedded_config_t const* config)
{
...
static void strpool_embedded_term(strpool_embedded_t* pool)
{
	int i;
...
static STRPOOL_EMBEDDED_U64 strpool_embedded_internal_make_handle(int index, int counter,
	STRPOOL_EMBEDDED_U64 index_mask, int counter_shift, STRPOOL_EMBEDDED_U64 counter_mask)
{
...
static int strpool_embedded_internal_counter_from_handle(STRPOOL_EMBEDDED_U64 handle,
	int counter_shift, STRPOOL_EMBEDDED_U64 counter_mask)
{
	return (int)((handle >> counter_shift) & counter_mask);
}

static int strpool_embedded_internal_index_from_handle(STRPOOL_EMBEDDED_U64 handle,
	STRPOOL_EMBEDDED_U64 index_mask)
{
	return ((int)(handle & index_mask)) - 1;
}

static strpool_embedded_internal_entry_t* strpool_embedded_internal_get_entry(
	strpool_embedded_t const* pool, STRPOOL_EMBEDDED_U64 handle)
{
...
static STRPOOL_EMBEDDED_U32 strpool_embedded_internal_find_in_blocks(
	strpool_embedded_t const* pool, char const* string, int length)
{
	int i;
	for (i = 0; i < pool->block_count; ++i)
	{
...
static char* strpool_embedded_internal_get_data_storage(strpool_embedded_t* pool, int size, int* alloc_size)
{
	...
	{
		char* ret = pool->blocks[pool->current_block].tail;
		pool->blocks[pool->current_block].tail += size;
		*alloc_size = size;
		return ret;
	}

	pool->current_block = strpool_embedded_internal_add_block(pool,
		size > pool->block_size ? size : pool->block_size);
...
static STRPOOL_EMBEDDED_U64 strpool_embedded_inject(strpool_embedded_t* pool, char const* string, int length)
{
	STRPOOL_EMBEDDED_U32 hash;
	int base_slot;
	int base_count;
	...
	while (base_count > 0)
	{
		...
		if (slot_base == base_slot)
		{
			...
			if (slot_hash == hash)
			{
				...
				strpool_embedded_internal_entry_t* e = &pool->entries[index];
				if (e->length == length &&
					(
						(!pool->ignore_case && STRPOOL_EMBEDDED_MEMCMP(e->data + 2 * sizeof(STRPOOL_EMBEDDED_U32), string, (size_t)length) == 0)
						|| (pool->ignore_case && STRPOOL_EMBEDDED_STRNICMP(e->data + 2 * sizeof(STRPOOL_EMBEDDED_U32), string, (size_t)length) == 0)
						))
				{
					int hi = e->handle_index;
					return strpool_embedded_internal_make_handle(hi, pool->handles[hi].counter,
	...
	STRPOOL_EMBEDDED_ASSERT(!pool->hash_table[slot].hash_key &&
		(hash & ((STRPOOL_EMBEDDED_U32)pool->hash_capacity - 1)) == (STRPOOL_EMBEDDED_U32)base_slot);
...
static char const* strpool_embedded_cstr(strpool_embedded_t const* pool, STRPOOL_EMBEDDED_U64 handle)
{
	strpool_embedded_internal_entry_t const* entry = strpool_embedded_internal_get_entry(pool, handle);
	if (entry) return entry->data + 2 * sizeof(STRPOOL_EMBEDDED_U32);
	return NULL;
}
...
#define CUTE_TILED_CHECK(X, Y) \
	do { if (!(X)) { cute_tiled_error_reason = Y; cute_tiled_error_cline = __LINE__; goto cute_tiled_err; } } while (0)

#define CUTE_TILED_FAIL_IF(X) \
	do { if (X) { goto cute_tiled_err; } } while (0)

static void* cute_tiled_alloc(cute_tiled_map_internal_t* m, int size)
{
	void* data;
	if (size > m->page_size) return NULL;
...
static CUTE_TILED_U64 cute_tiled_FNV1a(const void* buf, int len)
{
...
static char cute_tiled_parse_char(char c)
{
	switch (c)
	{
	case '\\': return '\\';
...
#define cute_tiled_skip_object(m) \
	do { CUTE_TILED_FAIL_IF(!cute_tiled_skip_object_internal(m)); } while (0)
...
#define cute_tiled_skip_array(m) \
	do { CUTE_TILED_FAIL_IF(!cute_tiled_skip_array_internal(m)); } while (0)
...
#define cute_tiled_read_int(m, num) \
	do { CUTE_TILED_FAIL_IF(!cute_tiled_read_int_internal(m, num)); } while (0)
...
static uint32_t cute_tiled_read_hex_int_internal(cute_tiled_map_internal_t* m, uint32_t* out)
{
	...
	while (m->in != end) { m->in++; val_length++; }
	*out = (uint32_t)val;
...
#define cute_tiled_read_hex_int(m, num) \
	do { CUTE_TILED_FAIL_IF(!cute_tiled_read_hex_int_internal(m, num)); } while (0)

static int cute_tiled_read_float_internal(cute_tiled_map_internal_t* m, float* out)
{
	char* end;
	float val = (float)strtod(m->in, &end);
	CUTE_TILED_CHECK(m->in != end, "Invalid float found during parse.");
	m->in = end;
	*out = val;
	return 1;
cute_tiled_err:
...
#define cute_tiled_read_float(m, num) \
	do { CUTE_TILED_FAIL_IF(!cute_tiled_read_float_internal(m, num)); } while (0)
...
#define cute_tiled_read_bool(m, b) \
	do { CUTE_TILED_FAIL_IF(!cute_tiled_read_bool_internal(m, b)); } while (0)

static int cute_tiled_read_csv_integers_internal(cute_tiled_map_internal_t* m, int* count_out, int** out)
{
	int count = 0;
	int capacity = 1024;
...
#define cute_tiled_read_csv_integers(m, count_out, out) \
	do { CUTE_TILED_FAIL_IF(!cute_tiled_read_csv_integers_internal(m, count_out, out)); } while (0)

static int cute_tiled_intern_string_internal(cute_tiled_map_internal_t* m, cute_tiled_string_t* out)
{
...
#define cute_tiled_intern_string(m, out) \
	do { CUTE_TILED_FAIL_IF(!cute_tiled_intern_string_internal(m, out)); } while (0)

static int cute_tiled_read_vertex_array_internal(cute_tiled_map_internal_t* m, int* out_count, float** out_verts)
{
	int vert_count = 0;
	int capacity = 32;
	float* verts;
...
#define cute_tiled_read_vertex_array(m, out_count, out_verts) \
	do { CUTE_TILED_FAIL_IF(!cute_tiled_read_vertex_array_internal(m, out_count, out_verts)); } while (0)

static int cute_tiled_skip_until_after_internal(cute_tiled_map_internal_t* m, char c)
{
	while (*m->in != c)
	{
...
#define cute_tiled_skip_until_after(m, c) \
	do { CUTE_TILED_FAIL_IF(!cute_tiled_skip_until_after_internal(m, c)); } while (0)

static int cute_tiled_read_properties_internal(cute_tiled_map_internal_t* m,
	cute_tiled_property_t** out_properties, int* out_count)
{
	int count = 0;
	int capacity = 32;
...
#define cute_tiled_read_properties(m, out_properties, out_count) \
	do { CUTE_TILED_FAIL_IF(!cute_tiled_read_properties_internal(m, out_properties, out_count)); } while (0)

static cute_tiled_object_t* cute_tiled_read_object(cute_tiled_map_internal_t* m)
{
	...
	while (cute_tiled_peak(m) != '}')
	{
		...
		case 14479365350473253539U: /* ellipse */
			cute_tiled_read_bool(m, &object->ellipse); break;
		case 14992147199312073281U: /* gid */
			cute_tiled_read_int(m, &object->gid); break;
		case 809651598226485190U:   /* height */
			cute_tiled_read_float(m, &object->height); break;
		case 3133932603199444032U:  /* id */
			cute_tiled_read_int(m, &object->id); break;
		case 12661511911333414066U: /* name */
			cute_tiled_intern_string(m, &object->name); break;
		case 15925463322410838979U: /* point */
			cute_tiled_read_bool(m, &object->point); break;
		case 11191351929714760271U: /* polyline */
			cute_tiled_read_vertex_array(m, &object->vert_count, &object->vertices);
			object->vert_type = 0; break;
		case 6623316362411997547U:  /* polygon */
			cute_tiled_read_vertex_array(m, &object->vert_count, &object->vertices);
			object->vert_type = 1; break;
		case 8368542207491637236U:  /* properties */
			cute_tiled_read_properties(m, &object->properties, &object->property_count); break;
		case 17386473859969670701U: /* rotation */
			cute_tiled_read_float(m, &object->rotation); break;
		...
		case 13509284784451838071U: /* type */
			cute_tiled_intern_string(m, &object->type); break;
		case 128234417907068947U:   /* visible */
			cute_tiled_read_bool(m, &object->visible); break;
		case 7400839267610537869U:  /* width */
			cute_tiled_read_float(m, &object->width); break;
		case 644252274336276709U:   /* x */
			cute_tiled_read_float(m, &object->x); break;
		case 643295699219922364U:   /* y */
			cute_tiled_read_float(m, &object->y); break;
...
static cute_tiled_layer_t* cute_tiled_layers(cute_tiled_map_internal_t* m)
{
	...
	while (cute_tiled_peak(m) != '}')
	{
		...
		{
		case 1485919047363370797U:  /* class */
			cute_tiled_intern_string(m, &layer->class_); break;
		case 14868627273436340303U: /* compression */
			CUTE_TILED_CHECK(0, "Compression is not yet supported. The expected tile format is CSV (uncompressed)."); break;
		case 4430454992770877055U:  /* data */
			CUTE_TILED_CHECK(cute_tiled_peak(m) == '[',
				"The expected tile format is CSV (uncompressed). It looks like Base64 was selected.");
			cute_tiled_expect(m, '[');
			cute_tiled_read_csv_integers(m, &layer->data_count, &layer->data); break;
		case 1888774307506158416U:  /* encoding */
			CUTE_TILED_CHECK(0, "Encoding is not yet supported. The expected tile format is CSV (uncompressed)."); break;
		case 2841939415665718447U:  /* draworder */
			cute_tiled_intern_string(m, &layer->draworder); break;
		case 809651598226485190U:   /* height */
			cute_tiled_read_int(m, &layer->height); break;
		case 13522647194774232494U: /* image */
			cute_tiled_intern_string(m, &layer->image); break;
		case 4566956252693479661U:  /* layers */
			...
			cute_tiled_expect(m, ']'); break;
		case 12661511911333414066U: /* name */
			cute_tiled_intern_string(m, &layer->name); break;
		...
		case 5195853646368960386U:  /* offsetx */
			cute_tiled_read_float(m, &layer->offsetx); break;
		case 5196810221485314731U:  /* offsety */
			cute_tiled_read_float(m, &layer->offsety); break;
		case 11746902372727406098U: /* opacity */
			cute_tiled_read_float(m, &layer->opacity); break;
		case 8368542207491637236U:  /* properties */
			cute_tiled_read_properties(m, &layer->properties, &layer->property_count); break;
		...
		case 1211175872446544425U:  /* tintcolor */
			...
			cute_tiled_expect(m, '"'); break;
		case 13509284784451838071U: /* type */
			cute_tiled_intern_string(m, &layer->type); break;
		case 128234417907068947U:   /* visible */
			cute_tiled_read_bool(m, &layer->visible); break;
		case 7400839267610537869U:  /* width */
			cute_tiled_read_int(m, &layer->width); break;
		case 644252274336276709U:   /* x */
			cute_tiled_read_int(m, &layer->x); break;
		case 643295699219922364U:   /* y */
			cute_tiled_read_int(m, &layer->y); break;
		case 18212633776084966362U: /* parallaxx */
			cute_tiled_read_float(m, &layer->parallaxx); break;
		case 18213590351201320707U: /* parallaxy */
			cute_tiled_read_float(m, &layer->parallaxy); break;
		case 222650047040294978U:   /* repeatx */
			cute_tiled_read_bool(m, &layer->repeatx); break;
		case 223606622156649323U:   /* repeaty */
			cute_tiled_read_bool(m, &layer->repeaty); break;
		case 3133932603199444032U:  /* id */
			cute_tiled_read_int(m, &layer->id); break;
		default:
			CUTE_TILED_CHECK(0, "Unknown identifier found.");
		}
...
static int cute_tiled_read_animation_frames_internal(cute_tiled_map_internal_t* m,
	cute_tiled_frame_t** out_frames, int* out_count)
{
	int count = 0;
	int capacity = 32;
...
#define cute_tiled_read_animation_frames(m, out_frames, out_count) \
	do { CUTE_TILED_FAIL_IF(!cute_tiled_read_animation_frames_internal(m, out_frames, out_count)); } while (0)

static cute_tiled_tile_descriptor_t* cute_tiled_read_tile_descriptor(cute_tiled_map_internal_t* m)
{
	...
	while (cute_tiled_peak(m) != '}')
	{
		...
		{
		case 3133932603199444032U:  /* id */
			cute_tiled_read_int(m, &tile_descriptor->tile_index); break;
		case 13509284784451838071U: /* type */
			cute_tiled_intern_string(m, &tile_descriptor->type); break;
		case 13522647194774232494U: /* image */
			cute_tiled_intern_string(m, &tile_descriptor->image); break;
		case 7796197983149768626U:  /* imagewidth */
			cute_tiled_read_int(m, &tile_descriptor->imagewidth); break;
		case 2114495263010514843U:  /* imageheight */
			cute_tiled_read_int(m, &tile_descriptor->imageheight); break;
		case 8368542207491637236U:  /* properties */
			cute_tiled_read_properties(m, &tile_descriptor->properties, &tile_descriptor->property_count); break;
		...
		case 6875414612738028948U:  /* probability */
			cute_tiled_read_float(m, &tile_descriptor->probability); break;
		case 2784044778313316778U:  /* terrain */
			cute_tiled_skip_array(m); break;
		case 3115399308714904519U:  /* animation */
			cute_tiled_read_animation_frames(m, &tile_descriptor->animation, &tile_descriptor->frame_count); break;
		default:
			CUTE_TILED_CHECK(0, "Unknown identifier found.");
		}
		cute_tiled_try(m, ',');
...
static int cute_tiled_read_point_internal(cute_tiled_map_internal_t* m, int* point_x, int* point_y)
{
	*point_x = 0;
	*point_y = 0;
	...
	while (cute_tiled_peak(m) != '}')
	{
		...
		{
		case 644252274336276709U: /* x */
			cute_tiled_read_int(m, point_x); break;
		case 643295699219922364U: /* y */
			cute_tiled_read_int(m, point_y); break;
		default:
			CUTE_TILED_CHECK(0, "Unknown identifier found.");
		}
		cute_tiled_try(m, ',');
...
#define cute_tiled_read_point(m, x, y) \
	do { CUTE_TILED_FAIL_IF(!cute_tiled_read_point_internal(m, x, y)); } while (0)
...
static cute_tiled_tileset_t* cute_tiled_tileset(cute_tiled_map_internal_t* m)
{
	...
	while (cute_tiled_peak(m) != '}')
	{
		...
		case 17465100621023921744U: /* backgroundcolor */
			...
			cute_tiled_expect(m, '"'); break;
		case 1485919047363370797U:  /* class */
			cute_tiled_intern_string(m, &tileset->class_); break;
		case 12570673734542705940U: /* columns */
			cute_tiled_read_int(m, &tileset->columns); break;
		case 13648382824248632287U: /* editorsettings */
			cute_tiled_skip_object(m); break;
		case 13956389100366699181U: /* firstgid */
			cute_tiled_read_int(m, &tileset->firstgid); break;
		case 16920059161811221315U: /* grid */
			cute_tiled_skip_object(m); break;
		case 13522647194774232494U: /* image */
			cute_tiled_intern_string(m, &tileset->image); break;
		case 7796197983149768626U:  /* imagewidth */
			cute_tiled_read_int(m, &tileset->imagewidth); break;
		case 2114495263010514843U:  /* imageheight */
			cute_tiled_read_int(m, &tileset->imageheight); break;
		case 4864566659847942049U:  /* margin */
			cute_tiled_read_int(m, &tileset->margin); break;
		case 12661511911333414066U: /* name */
			cute_tiled_intern_string(m, &tileset->name); break;
		case 1007832939408977147U:  /* tiledversion */
			cute_tiled_intern_string(m, &tileset->tiledversion); break;
		case 8196820454517111669U:  /* version */
			if (*m->in == '"') m->in++;
			cute_tiled_read_float(m, &tileset->version);
			if (*m->in == '"') m->in++; break;
		case 8368542207491637236U:  /* properties */
			cute_tiled_read_properties(m, &tileset->properties, &tileset->property_count); break;
		case 6491372721122724890U:  /* spacing */
			cute_tiled_read_int(m, &tileset->spacing); break;
		case 4065097716972592720U:  /* tilecount */
			cute_tiled_read_int(m, &tileset->tilecount); break;
		case 13337683360624280154U: /* tileheight */
			cute_tiled_read_int(m, &tileset->tileheight); break;
		case 2769630600247906626U:  /* tileoffset */
			cute_tiled_read_point(m, &tileset->tileoffset_x, &tileset->tileoffset_y); break;
		case 7277156227374254384U:  /* tileproperties */
			CUTE_TILED_WARNING("`tileproperties` is deprecated. Attempting to skip.");
			CUTE_TILED_FAIL_IF(cute_tiled_skip_curly_braces_internal(m)); break;
		...
		case 6504415465426505561U:  /* tilewidth */
			cute_tiled_read_int(m, &tileset->tilewidth); break;
		case 8489814081865549564U:  /* transparentcolor */
			cute_tiled_expect(m, '"');
			cute_tiled_read_hex_int(m, &tileset->transparentcolor);
			cute_tiled_expect(m, '"'); break;
		case 13509284784451838071U: /* type */
			cute_tiled_intern_string(m, &tileset->type); break;
		...
		case 1819203229U:           /* objectalignment */
			cute_tiled_intern_string(m, &tileset->objectalignment); break;
		...
		case 14766449174202642533U: /* terrains */
			cute_tiled_skip_array(m); break;
		case 6029584663444593209U:  /* wangsets */
			cute_tiled_skip_array(m); break;
		default:
			CUTE_TILED_CHECK(0, "Unknown identifier found.");
		}
...
static int cute_tiled_dispatch_map_internal(cute_tiled_map_internal_t* m)
{
	...
	case 17465100621023921744U: /* backgroundcolor */
		...
		cute_tiled_expect(m, '"'); break;
	case 1485919047363370797U:  /* class */
		cute_tiled_intern_string(m, &m->map.class_); break;
	...
	case 13648382824248632287U: /* editorsettings */
		cute_tiled_skip_object(m); break;
	case 809651598226485190U:   /* height */
		cute_tiled_read_int(m, &m->map.height); break;
	case 16529928297377797591U: /* infinite */
		cute_tiled_read_bool(m, &m->map.infinite); break;
	case 4566956252693479661U:  /* layers */
		cute_tiled_expect(m, '[');
		while (cute_tiled_peak(m) != ']')
		{
	...
	case 11291153769551921430U: /* nextobjectid */
		cute_tiled_read_int(m, &m->map.nextobjectid); break;
	case 563935667693078739U:   /* orientation */
		cute_tiled_intern_string(m, &m->map.orientation); break;
	case 8368542207491637236U:  /* properties */
		cute_tiled_read_properties(m, &m->map.properties, &m->map.property_count); break;
	case 16693886730115578029U: /* renderorder */
		cute_tiled_intern_string(m, &m->map.renderorder); break;
	case 1007832939408977147U:  /* tiledversion */
		cute_tiled_intern_string(m, &m->map.tiledversion); break;
	case 13337683360624280154U: /* tileheight */
		cute_tiled_read_int(m, &m->map.tileheight); break;
	case 8310322674355535532U:  /* tilesets */
	{
		cute_tiled_expect(m, '[');
		while (cute_tiled_peak(m) != ']')
		{
			cute_tiled_tileset_t* ts = cute_tiled_tileset(m);
			CUTE_TILED_FAIL_IF(!ts);
			ts->next = m->map.tilesets;
			m->map.tilesets = ts;
			cute_tiled_try(m, ',');
		}
		cute_tiled_expect(m, ']');
	...
	case 6504415465426505561U:  /* tilewidth */
		cute_tiled_read_int(m, &m->map.tilewidth); break;
	case 13509284784451838071U: /* type */
		cute_tiled_intern_string(m, &m->map.type); break;
	case 8196820454517111669U:  /* version */
		if (*m->in == '"') m->in++;
		cute_tiled_read_float(m, &m->map.version);
		if (*m->in == '"') m->in++; break;
	case 7400839267610537869U:  /* width */
		cute_tiled_read_int(m, &m->map.width); break;
	case 2498445529143042872U:  /* nextlayerid */
		cute_tiled_read_int(m, &m->map.nextlayerid); break;
	default:
		CUTE_TILED_CHECK(0, "Unknown identifier found.");
	}
	return 1;
...
#define cute_tiled_dispatch_map(m) \
	do { CUTE_TILED_FAIL_IF(!cute_tiled_dispatch_map_internal(m)); } while (0)

/* -----------------------------------------------------------------------
   String de-interning (second pass)
----------------------------------------------------------------------- */
...
static void cute_tiled_deintern_properties(cute_tiled_map_internal_t* m,
	cute_tiled_property_t* properties, int property_count)
{
	int i;
...
static cute_tiled_map_internal_t* cute_tiled_map_internal_alloc_internal(
	void* memory, int size_in_bytes, void* mem_ctx)
{
...
cute_tiled_tileset_t* cute_tiled_load_external_tileset(const char* path, void* mem_ctx)
{
	...
	ts = cute_tiled_load_external_tileset_from_memory(file, size, mem_ctx);
	CUTE_TILED_FREE(file, mem_ctx);
	cute_tiled_error_file = NULL;
	return ts;
}
...
cute_tiled_tileset_t* cute_tiled_load_external_tileset_from_memory(const void* memory, int size_in_bytes, void* mem_ctx)
{
	...
	cute_tiled_tileset_t* ts = cute_tiled_tileset(m);
	cute_tiled_patch_tileset_strings(m, ts);
	CUTE_TILED_REVERSE_LIST(cute_tiled_tile_descriptor_t, ts->tiles);
	ts->_internal = m;
	return ts;
}
